...
export class SolidityCompiler {
  ...
  async compile(sourceCode, contractName = 'Contract', options = {}) {
    ...
    try {
      ...
      const fileName = `${contractName}_${Date.now()}.sol`;
      ...
      const command = `solc ${compileOptions} "${tempFile}"`;
      ...
      if (stderr) {
        logger.warn(`Solidity compilation warnings: ${stderr}`);
      }
      ...
      logger.info(`Solidity compilation completed in ${compilationTime}ms`);
      ...
      logger.error(`Solidity compilation failed: ${error.message}`);
